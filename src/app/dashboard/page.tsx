                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-xs text-gray-500">
                            {format(new Date(nextExam.date), 'MMM d, yyyy')} · {nextExam.venue}
                          </span>
                        </div>
                      </div>
                    </div>
                    <CountdownTimer date={new Date(nextExam.date)} />
                  </div>
                </Card>
              </motion.div>
            )}
            ...
            <motion.div variants={item}>
              <Card>
                ...
                <div className="space-y-3">
                  {homework.map((hw) => {
                    const daysLeft = differenceInDays(new Date(hw.dueDate), new Date());
                    const hoursLeft = differenceInHours(new Date(hw.dueDate), new Date());
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-3 text-[11px] text-gray-500">
                              ...
                              <span className="flex items-center gap-1">
                                ...
                                {isOverdue
                                  ? 'Was due ' + formatDistanceToNow(new Date(hw.dueDate), { addSuffix: true })
                                  : hoursLeft < 24
                  ...
                  {exams.map((exam) => (
                    <div key={exam.id} className="flex items-center gap-3 p-3 rounded-xl border border-white/[0.06] bg-white/[0.02]">
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-red-500/10 to-orange-500/10 border border-red-500/10 flex flex-col items-center justify-center">
                        <span className="text-xs font-bold text-red-400">{format(new Date(exam.date), 'dd')}</span>
                        <span className="text-[10px] text-red-400/70">{format(new Date(exam.date), 'MMM')}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="text-right">
                        <p className="text-xs text-gray-400">{differenceInDays(new Date(exam.date), new Date())}d</p>
                      </div>
                    </div>
                  ))}
                </div>
